...
export default function EditCriteriaForm({ criterion, tipeOptions, onCancel }) {
    const { data, setData, put, processing, errors } = useForm({
        nama_kriteria: criterion.nama_kriteria,
        tipe: criterion.tipe,
        kategori: criterion.kategori || 'product',
        options: criterion.options || ['', '', '', '', ''],
        satuan: criterion.satuan || '',
    });

    const submit = (e) => {
        e.preventDefault();
        put(route('criteria.update', criterion.id), { onSuccess: onCancel });
    };

    return (
        <div className="p-6">
            <h2 className="text-lg font-medium text-navy-900 mb-6">Edit Kriteria</h2>
            <form onSubmit={submit} className="space-y-4">
                <div>
                    <InputLabel htmlFor="edit_nama_kriteria" value="Nama Kriteria" />
                    <TextInput
                ...
                <div>
                    <InputLabel htmlFor="edit_kategori" value="Kategori" />
                    <select
                        id="edit_kategori"
                        value={data.kategori}
                ...
                <div>
                    <InputLabel htmlFor="edit_tipe" value="Tipe" />
                    <select
                        id="edit_tipe"
                        value={data.tipe}
                        className="mt-1 block w-full rounded-md border-navy-300 shadow-sm focus:border-primary-500 focus:ring-primary-500"
                ...
                {data.tipe === 'cost' && (
                    <div>
                        <InputLabel htmlFor="edit_satuan" value="Satuan (Opsional)" />
                        <TextInput
                            id="edit_satuan"
                            value={data.satuan}
                            className="mt-1 block w-full"
